...
router.get('/me', authenticate, requireRole('PARENT'), async (req: AuthRequest, res) => {
  try {
...
router.get('/me/children', authenticate, requireRole('PARENT'), async (req: AuthRequest, res) => {
  try {
    const parent = await prisma.parent.findUnique({
      where: { userId: req.user!.id },
    })
...
router.get('/me/children/:childId/status', authenticate, requireRole('PARENT'), async (req: AuthRequest, res) => {
  try {
    const parent = await prisma.parent.findUnique({
      where: { userId: req.user!.id },
    })
...
router.patch('/me/children/:childId/attendance', authenticate, requireRole('PARENT'), async (req: AuthRequest, res) => {
  try {
...
router.patch('/me/children/:childId/location', authenticate, requireRole('PARENT'), async (req: AuthRequest, res) => {
  try {
    const { childId } = req.params
