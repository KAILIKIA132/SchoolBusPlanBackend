...
router.get('/me', authenticate, requireRole('DRIVER'), async (req: AuthRequest, res) => {
  try {
...
router.get('/me/students', authenticate, requireRole('DRIVER'), async (req: AuthRequest, res) => {
  try {
    const driver = await prisma.driver.findUnique({
      where: { userId: req.user!.id },
      include: { bus: true },
...
router.get('/me/trips', authenticate, requireRole('DRIVER'), async (req: AuthRequest, res) => {
  try {
    const driver = await prisma.driver.findUnique({
      where: { userId: req.user!.id },
    })
...
router.post('/pickup/:studentId', authenticate, requireRole('DRIVER'), async (req: AuthRequest, res) => {
  try {
...
router.post('/dropoff/:studentId', authenticate, requireRole('DRIVER'), async (req: AuthRequest, res) => {
  try {
    const { studentId } = req.params
...
router.patch('/:id', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (busId !== undefined) updateData.busId = busId || null
